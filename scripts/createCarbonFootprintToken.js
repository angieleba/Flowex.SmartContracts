...
async function main() {
    ...
    const transaction = await new TokenCreateTransaction()
        .setTokenName("Floex Carbon Footprint Token")
        .setTokenSymbol("FXCFT")
        .setTreasuryAccountId(operatorId)
        .setInitialSupply(10000000000000)
        .setDecimals(5)
        .setAutoRenewAccountId(operatorId)
        .setAutoRenewPeriod(6000000)
        .setMaxTransactionFee(new Hbar(13))
        .freezeWith(client);
    ...
    console.log(`Flowex carbon footprint token smart contract ID in Solidity format is: ${tokenAddress} \n`);
    const contractBaseLink = "https://hashscan.io/testnet/contract/";
    fs.writeFileSync(
        "CFTokenData.json",
        JSON.stringify({
            CFTokenAddress: tokenAddress,
            CFTokenContractLink: contractBaseLink + tokenId,
        }, null, 4)
    );
}
